				...
				for($i=0; $i<$Entries['count']; $i++) 
					...
					switch($this->Attributes['order_format'][$j])
						...
						case 'ad_def_full_name':
							$ArrSorted[$i]=preg_replace("/([Ñ‘A-zÐ-Ñ-]+[\s]{1}[Ñ‘A-zA-Ñ]{1}.)[\s]{1}([Ñ‘A-zÐ-Ñ-]+)/u", "\\2 \\1", $ArrSorted[$i]);
						break;
				...
				for($j=0; $j<$SizeOf; $j++)
					...
					if(is_array(@$this->PregReplace[$this->Attributes[title][$j]]['conditions']))
						...
						foreach(@$this->PregReplace[$this->Attributes[title][$j]]['conditions'] as $key=>$value)
							...
							//$PreviousFlag=true;
							foreach(@$this->PregReplace[$this->Attributes[title][$j]]['conditions'][$key] as $key1=>$value1) //Ð¦Ð¸ÐºÐ» Ð¿Ð¾ ÐºÐ»ÑŽÑ‡Ð°Ð¼ ldap
								{
...
class LDAP
	...
	function getValue($DN, $Attribute, $Filter=false, $NotRecode=false) //Ð£ÑÑ‚Ð°Ñ€ÐµÐ»Ð°. Ð˜ÑÐ¿Ð¾Ð»ÑŒÐ·Ð¾Ð²Ð°Ñ‚ÑŒ getAttrValue()
		{
		$Attributes=array($Attribute);
						...
						if(is_array($val))
							...
							foreach($val AS $key1=>$val1)
								...
								if(is_array($val1))
									...
									foreach($val1 AS $key2=>$val2)
										...
										switch($val2)
											...
											case 'order_replace':
												$d.=$key2;
												$LastVal[$key-1]=str_replace($key1, $d, $LastVal[$key-1]); //!!!! Ð’Ð¾Ð·Ð¼Ð¾Ð¶Ð½Ð¾ Ð´ÐµÑÐ½Ð¸Ðµ Ð´Ð¾Ð»Ð¶Ð½Ð¾ Ð±Ñ‹Ñ‚ÑŒ Ð´Ñ€ÑƒÐ³Ð¸Ð¼
											break;										
											}	
										}
									}
								...
								switch($val1)
									{
									case 'ad_def_full_name':
										$LastVal[$key-1]=preg_replace("/([Ñ‘A-zÐ-Ñ-]+[\s]{1}[Ñ‘A-zÐ-Ñ]{1}.)[\s]{1}([Ñ‘A-zÐ-Ñ-]+)/u", "\\2 \\1", $LastVal[$key-1]);
